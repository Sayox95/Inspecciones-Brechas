...
<head>
  ...
  <style>
    :root{
      --bg:#0b1220;--card:#0f182a;--muted:#8aa0b6;--text:#ecf3ff;--accent:#4cc9f0;--ok:#22c55e;--warn:#f97316;--danger:#ef4444;--border:#1d2a45;--sheet:#0b1220;--radius:16px;--shadow:0 10px 30px rgba(0,0,0,.35);
      --ronda:#7c3aed; /* morado para diferenciar rondas */
    }
    *{box-sizing:border-box} html,body{height:100%}
    ...
    .fab-wrap{position:fixed;right:16px;bottom:16px;z-index:1100;display:flex;flex-direction:column;gap:10px}
...
<body>
  ...
  <section id="sheet" class="sheet" aria-modal="true" role="dialog">
    ...
    <div class="content">
      ...
      <!-- PASO 1 (dinámico por modo) -->
      <div id="step-1" class="grid step active">
        <div class="card">
          <div id="chipsTipo" class="chips" style="margin-bottom:8px">
          ...
          <div class="step1-two-col">
            <div class="step1-col">
              ...
              <!-- Circuito solo visible en TRAMO (en RONDA se pedirá en el paso 2) -->
              <div class="field" id="wrap-circuito-s1">
                <label>Circuito</label>
                <select id="circuito" disabled><option value="">Selecciona una subestación…</option></select>
              </div>
            </div>
            <div class="step1-col">
              <!-- Controles para TRAMO -->
              ...
              <!-- Control exclusivo para RONDA (cantidad de postes rondados) -->
              <div class="field ronda-only hidden">
                <label for="postesRondados">Postes rondados (cantidad)</label>
      ...
      <div id="step-2" class="grid step hidden">
        <div class="card grid">
          <!-- En RONDA, pedir Circuito aquí -->
          <div class="field ronda-only hidden">
            <label>Circuito</label>
            <select id="circuitoRonda" disabled><option value="">Selecciona una subestación…</option></select>
          </div>
  ...
  const html = `
    <div style="text-align:left">
      ...
      <ul style="padding-left:20px;font-size:13px;line-height:1.4em">
        <li><b>Android (Chrome):</b> toca el candado 🔒 en la barra de direcciones → <em>Permisos</em> → <b>Ubicación → Permitir</b>.</li>
        <li><b>iPhone (Safari):</b> Ajustes → Safari → Ubicación → <b>Preguntar o Permitir</b>.</li>
        <li><b>PC:</b> Toca el ícono del candado 🔒 → <b>Permitir ubicación</b>.</li>
      </ul>
      <p>Luego recarga la página 🔄.</p>
      <button class="btn btn-primary" onclick="location.reload()">Recargar ahora</button>
    </div>`;
  showBlocker("Permisos GPS");
  document.querySelector("#blocker .box").innerHTML = html;
}
...
function baseActual(){return { mode:'tramo', /* 'tramo' | 'ronda' */
  sector:"", subestacion:"", circuito:"", tipo:"Apertura", troncalRamal:"", terna:"",
  postesRondados:0, inicio:null, fin:null, fotoInicioFile:null, fotoFinFile:null };}
...
function setStep(n){
  ...
  nextEl.addEventListener('animationend', function onEnter(){ nextEl.classList.remove(enterClass); }, { once: true });
  step = n;
}
...
function tipoColor(it){
  if (it.mode==='ronda') return getComputedStyle(document.documentElement).getPropertyValue('--ronda').trim() || '#7c3aed';
  return it.tipo==='Limpieza'?'#22c55e':'#f97316';
}
...
function applyModeUI(mode){
  ...
  // Circuito en paso 1 (solo tramo)
  $('wrap-circuito-s1').classList.toggle('hidden', isRonda);
}

function validateStep1(){
  const sector=$('sector').value.trim(), sub=$('subestacion').value.trim();
  const isRonda = state.actual.mode==='ronda';
  if(isRonda){
    const cantidad = parseInt(($('postesRondados').value||'').trim(),10);
    if(!sector||!sub||!(cantidad>=0)) return false;
    Object.assign(state.actual,{sector,subestacion:sub,postesRondados: isNaN(cantidad)?0:cantidad});
    return true;
  }else{
    const circ=$('circuito').value.trim();
...
async function validateStep2(){
  // En RONDA, asegurar circuito si aún no se eligió (se muestra en este paso)
  if (state.actual.mode==='ronda') {
    const circ = $('circuitoRonda').value.trim();
    if(!circ){ toast('Selecciona el circuito.'); return false; }
    state.actual.circuito = circ;
  }
...
function fillResumen(){
  ...
  if (it.mode==='ronda'){
    $('resumen').innerHTML=`<div class="grid">
      ...
      <div><b>Subestación:</b> ${it.subestacion}</div>
      <div><b>Circuito:</b> ${it.circuito||'-'}</div>
      <div><b>Postes rondados:</b> ${it.postesRondados}</div>
    ...
    $('resumen').innerHTML=`<div class="grid">
      ...
      <div><b>Troncal/Ramal:</b> ${it.troncalRamal}</div>
      <div><b>Terna:</b> ${it.terna}</div>
      <div><b>Inicio:</b> ${ini}</div>
      <div><b>Final:</b> ${fin}</div>
    </div>`;
  }
...
document.getElementById('s4-save').onclick = async ()=>{
  ...
  if (state.actual.mode==='ronda' && !state.actual.circuito){
  ...
  try {
    ...
    const it = {
      ...
      mode: state.actual.mode, // 'tramo' | 'ronda'
...
$('newTramo').onclick=()=>{
  clearWizard(true);
  state.actual.mode='tramo';
  applyModeUI('tramo');
  openSheet(); setStep(1);
};
$('newRonda').onclick=()=>{
  clearWizard(true);
  state.actual.mode='ronda';
  applyModeUI('ronda');
  openSheet(); setStep(1);
};
...
$('s1-next').onclick=()=>{
  if(!validateStep1()){
    toast(state.actual.mode==='ronda' ? 'Completa Sector, Subestación y cantidad de postes.' : 'Completa todos los campos.');
    return;
  }
  setStep(2);
};
...
function initCascadingSelects(){
  ...
  const selCircR = document.getElementById('circuitoRonda');
  ...
  selSector.onchange = ()=>{
    ...
    setSelectOptions(selCircR, [], 'Selecciona una subestación…');
    selCircR.disabled = true;
  ...
  selSub.onchange = ()=>{
    ...
    // Para RONDA (paso 2)
    setSelectOptions(selCircR, circs, circs.length ? 'Selecciona un circuito…' : 'Sin circuitos');
    selCircR.disabled = circs.length === 0;
  ...
  selCircR.onchange = ()=>{ state.actual.circuito = selCircR.value || ''; };

  // Preselección si existiera
  if (state.actual.sector){
    ...
    if (state.actual.subestacion){
      ...
      if (state.actual.circuito){
        if (state.actual.mode==='ronda'){ selCircR.value = state.actual.circuito; }
        else { selCirc.value = state.actual.circuito; }
      }
    }
  }
}
...
// Polyfills mínimos para compatibilidad
